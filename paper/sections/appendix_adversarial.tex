...
\textbf{Attack.} Without regularization, the router converges to always selecting a single endpoint, ignoring potentially better alternatives for specific task types.
...
The Product-of-Experts gate layer (Proposition~\ref{prop:poe}) provides additional stability: endpoints that fail hard constraints are eliminated before scoring, removing the most volatile rankings.

\subsection{Weight Manipulation via Feedback}

\textbf{Attack.} An adversary submits systematically biased feedback signals to steer the Robbins-Monro weight updates toward a preferred endpoint.
